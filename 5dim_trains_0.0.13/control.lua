...
game.on_event(defines.events.on_tick, function(event)
	...
	if global.itemsToDeleteFromInventory ~= nil then
		local playerInventory = game.player.get_inventory(defines.inventory.playermain)
		...
		if foundWagon ~= nil and foundWagon.valid then
			for i, wagonData in ipairs(global.wagonsData) do
				if hasValidWagon(wagonData) and foundWagon == wagonData.wagon then
					...
					wagonData.wagon.damage(10000, game.forces.enemy)
				end
			end
		end
...
game.on_event(defines.events.on_preplayer_mined_item, function(event) -- THIS SHOULD BE THE SAME AS events.on_entity_died - I should really create a new function...
	if isKnownWagon(event.entity.name) then
		removeWagonFromTable(global.wagonsData, event.entity)
	end
	if hasStaticProxies(event.entity.name) then
...
game.on_event(defines.events.on_robot_pre_mined, function(event) -- THIS SHOULD BE THE SAME AS events.on_entity_died - I should really create a new function...
	if isKnownWagon(event.entity.name) then
		removeWagonFromTable(global.wagonsData, event.entity)
	end
	if hasStaticProxies(event.entity.name) then
			...
			-- for i, entity in ipairs(entities) do
				-- if entity ~=nil and entity.valid then
					...
					-- -- if railInTheSameLocation(event.created_entity, entity) then
						...
						-- -- game.player.insert({name = event.created_entity.name, count = 1})
						-- -- --debugLog("destroy!")
...
-- game.on_event(defines.events.onplayermineditem, function(event)
...
-- game.on_event(defines.events.onpickedupitem, function(event) -- SAME AS onplayermineditem, SHOULD BE A FUNCTION!
	-- if event.itemstack["name"] == "nothing-item" then
		-- --debugLog("Found Nothing Item")
		-- if global.itemsToDeleteFromInventory == nil then
			-- global.itemsToDeleteFromInventory = {}
...
function findWagonAtPosition(position)
	...
	--local cargoWagons = game.find_entities{{position.x + 5, position.y - 5}, {position.x + 5, position.y + 5}}
