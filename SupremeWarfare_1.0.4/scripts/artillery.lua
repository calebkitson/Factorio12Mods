...
function fireArty(arty,target,distance,missile)
	...
	if atr.scatter then 
		...
		local newTarget=game.player.surface.create_entity({name="sb-target", position = {xS, yS}, force = game.forces.enemy})
		game.player.surface.create_entity({name=missile, target = newTarget, force = game.forces.player, position = arty.position, speed= mspeed})
		newTarget.destroy()
	else
		game.player.surface.create_entity({name=missile, position = {x = arty.position.x, y = arty.position.y}, force = game.forces.player, target = target, speed= mspeed})
	end
	if atr.muzzle then
		game.player.surface.create_entity({name="muzzle-flash", position = arty.position, force = game.forces.player, target = arty, speed= 0.01})
	end
	game.player.surface.pollute(arty.position,atr.pollution)
end

function pullAggro(arty,targetpos)
	--units = game.findenemyunits(targetpos, 15)
	units = game.player.surface.find_enemy_units(targetpos, 15)
...
function processArty(artyList)
	...
	if ammo > 0 then	
		if arty.name == "sb-artillery-mk1" then
			enemys = game.player.surface.find_enemy_units(arty.position, atr.maxrange)
			checkUnits=true
		end
		if atr.autofire then
			spawner = game.player.surface.find_entities_filtered({area = {{x = arty.position.x - atr.maxrange, y = arty.position.y - atr.maxrange}, {x = arty.position.x + atr.maxrange, y = arty.position.y + atr.maxrange}}, type = "unit-spawner"})
			--CheckUnits
